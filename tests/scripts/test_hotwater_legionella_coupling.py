...
only by a minimum days-since-last-cycle check (is_legionella_due). These
...
def _run_force_heat_check(tmp_path: Path, *, legionella_last_completed_days_ago: float | None, max_temp: float = 65.0):
    ...
    # daily_check_lookup_date_str), silently failing the legionella-due
    ...
    today_str = core.daily_check_lookup_date_str({}, now_local)
...
def test_legionella_never_runs_sooner_than_the_interval():
    ...
    assert core.is_legionella_due(hw_config, not_due_state) is False
    assert core.is_legionella_due(hw_config, due_state) is True
