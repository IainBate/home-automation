...
function hotWaterCard(d) {
  ...
  const body = `
    ...
    ${d.legionella_days_since_last != null && d.legionella_days_until_due != null && d.legionella_due_date != null ? `<div class="row"><span class="label">Legionella cycle</span><span class="value">${d.legionella_days_since_last}d ago · ${d.legionella_days_until_due > 0 ? `due on ${fmtDateDdMmYy(d.legionella_due_date)}` : `overdue since ${fmtDateDdMmYy(d.legionella_due_date)}`}</span></div>` : ""}
...
function airstageZoneCard(zone) {
  ...
  const auto = zone.hvac_automation;
  const automationRow = auto
    ? `<div class="row"><span class="label">Automation</span><span class="value"><span class="badge ${auto.away_mode_active ? "warn" : "good"}">${auto.away_mode_active ? "Away mode" : "Active"}</span></span></div>`
    : "";
  const body = `
