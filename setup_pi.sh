...
echo "  - Encrypted secrets backup: run 'bash scripts/encrypt_secrets.sh' once (enter this"
echo "      Pi's login password when prompted), paste the printed hash into"
echo "      secrets_backup.passphrase_hash in secrets.yaml, then add a daily cron entry:"
echo "      0 3 * * * cd $PROJECT_DIR && bash scripts/encrypt_secrets.sh --quiet"
